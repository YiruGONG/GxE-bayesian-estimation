...
g_ite = function(y, x, beta, sig, tau, l)
{
  # y:n*1 x:n*p beta:p*1 sig:number
  # tau,l:parameter
  n = length(y)
  p = length(beta)
...
beta_ite = function(y, x, beta, sig, tau, l, lamda=2000, var_b=10^(-3), sigma_b=1, proposal)
{
  ...
  else if (proposal=='t_proj') {
    # beta_new = rep(0,4)
    # for (i in 1:p){
    #   beta_new[i] = rnorm(n=1,0,sigma_b[i])
    # }
    beta_new = mvrnorm(n=1,rep(0,p),diag(sigma_b))
  }
  ### maybe delete
  else{
    beta_new = mvrnorm(n=1, beta, diag(rep(var_b,p)))
...
tao_ite = function(y, x, beta, tau, l,sig, var_tau, a=0.5, b=0.5)
{
  n = nrow(x)
  p = length(beta)
...
l_ite = function(y, x, beta, tau, l, sig, var_l, a=0.5, b=0.5)
{
  n = nrow(x)
  p = length(beta)
...
estimate_bayes = function( y, x, g0, beta0, sig0, tau0, l0, lamda=2000, var_b=10^(-8), sigma_b0 = 0.5, a=0.5, b=0.5, var_tau, var_l, ite, proposal)

{
  ...
  for(i in 1:ite)
  {
    ...
    # {
    #   beta_outp = b_ygv1(x=x, beta=beta[i,], y, sig=sig[i], tau=tau[i], l=l[i], var_b=var_b, sigma_b=sigma_b0, proposal=proposal)
    # }
    # else
    if (proposal=='t_proj') {
      beta_outp = beta_ite(y, x, beta[i,], sig[i], tau[i], l[i], var_b=var_b, sigma_b=sigma_b[i,] ,proposal=proposal)
    }
    else {
      beta_outp = beta_ite(y, x, beta[i,], sig[i], tau[i], l[i], lamda=lamda, var_b=var_b, proposal=proposal)
    }
    
    beta[i+1,] = beta_outp[[1]]
    accept_beta = accept_beta + beta_outp[[2]]

    tau_outp = tao_ite(y, x, beta[i+1,], tau[i], l[i], sig[i], var_tau)
    tau[i+1] = tau_outp[[1]]
    accept_tau = accept_tau + tau_outp[[2]]

    l_outp = l_ite(y, x, beta[i+1,], tau[i+1], l[i], sig[i], var_l)
    l[i+1] = l_outp[[1]]
    accept_l = accept_l + l_outp[[2]] 

    if(proposal=='t_proj')
    {
      sigma_b[i+1,] = sig_b_ite(beta[i+1,], a, b)
    }
    
    g[i+1,] = g_ite(y, x, beta[i+1,], sig[i], tau[i+1], l[i+1])

    sig[i+1] = sig_ite(y, g[i+1,])
  }

  if(proposal=='t_proj'){
    result = list(beta=beta,sigma_b=sigma_b, g=g, sig=sig, tau=tau, l=l, accept_rate=c(accept_beta,accept_tau,accept_l)/ite)
  } else {
    result = list(beta=beta,sigma_b=NA, g=g, sig=sig, tau=tau, l=l, accept_rate=c(accept_beta,accept_tau,accept_l)/ite)
  }
  return(result)
}
#-------------------------------------------------------------------------------------------------------
